...
## Update 2026-09-17 (2) — Critical: live request feed was broken for everyone; and two auth-race bugs found testing Google Sign-In

**1. CRITICAL, confirmed live, now fixed: `request_directory_public` had no grants — the request feed was broken for every guest and signed-in user.** Caught while regression-testing the Google Sign-In work below, via a direct anon-key REST probe unrelated to that testing (`GET /rest/v1/request_directory_public` → `42501 permission denied`). Root cause: `patch_25_public_data_privacy.sql`'s donor-directory section references `donor_directory_public.is_smoker`, a column `patch_26_remove_public_smoker_status.sql` later dropped. Re-running `patch_25` in full (to try to fix the grants) failed partway through with `column "is_smoker" does not exist` — and since that script has no transaction wrapper, the SQL Editor commits each statement as it runs, so everything after the failure point — including the entire request-feed section — never executed. Fixed with a new `patch_27_fix_request_directory_public_grants.sql`: exactly `patch_25`'s request-feed section (lines 118-243) copied standalone, nothing touching `is_smoker`/`donor_directory_public`. **Verified fixed live via the actual REST endpoints the app uses** (not just `has_table_privilege`, which reported `false` for the *base table* — expected, since that grant is column-level and the app only ever queries the views): `GET /rest/v1/v_public_requests` as anon → `200` with real data; `GET /rest/v1/v_authenticated_requests` as an authenticated user → `200` with `requester_id` included.
- **Prevention noted in the new patch**: `patch_25` combined two independent concerns (donor projection, request projection) in one non-transactional script — a failure in one silently skips the other. Split future projection patches by concern so this can't recur the same way.

**2. `link_or_get_my_donor()`'s null-shell result was silently treated as a found row, blocking sign-in for any account without an existing `donors` row.** Reported as "Your session needs a refresh" repeating on every submit of the new complete-profile form. Confirmed live: the RPC returns an object with every column (including `id`) set to `null` instead of actually finding/creating a row. `signInDonor()` and `getCurrentDonorFromSession()` both checked only truthiness of that result, so the null shell was mistaken for "found," skipping the fallback insert that would have produced a real row — the resulting profile's `id: null` then hit `handleLoginSuccess`'s existing null-id guard (`ea738cc`) and was silently dropped, every time. Fixed by checking `linked.data?.id` instead of `linked.data` in both places.
- **Related fix surfaced by this one**: `signInDonor()`'s own fallback insert had no recovery for losing a race to create the row first, unlike `getCurrentDonorFromSession()` which already had one — added the same select-and-retry-on-`23505` recovery for consistency.
- **Related race surfaced by this one**: right after a fresh signup, the auth-state listener now also reaches its own fallback insert (previously it silently no-op'd via the bug above) and can race `signUpDonor()`'s own insert — if the listener's bare-default insert lands first, `signUpDonor()`'s insert still recovers correctly server-side, but the listener's already-resolved blank profile could still overwrite `handleLoginSuccess`'s complete one client-side purely on timing (confirmed live: a fresh complete signup briefly showed the profile-completion gate it shouldn't have). Fixed with a guard in `App.tsx`'s listener callback: a same-id resolution with a blank `phone` never regresses an already-complete profile, mirroring the existing null-id guard for the same class of race.
- **Verified live, end to end**: a genuinely incomplete account (real auth user, no `donors` row — the exact shape a first-time Google/OAuth sign-in produces) now reaches and successfully submits the complete-profile form; a fresh complete signup no longer flashes the completion gate; normal re-sign-in into a complete account is unaffected.
- **Also confirmed correct, not a bug**: the complete-profile form's validation order (name → blood group → phone) was suspected wrong — reproduced directly and confirmed it fires in the intended order.
